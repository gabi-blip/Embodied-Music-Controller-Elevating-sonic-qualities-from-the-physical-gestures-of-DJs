...
char ssid[] = "gabi";          // your network SSID (name)
char pass[] = "gabigabi";         // your network password

// Define Network IP and UDP out port for data transmission
const IPAddress outIp1(172,20,10,4);      // Hotspot IP Address (Gabriel)
const IPAddress outIp2(192,168,43,176);      // Hotspot IP Address (Marco)
int selectedIP = 1;                          // 1 = Gabriel & 2 = Marco
...
// 2. Send OSC Messages over UDP
  ...
  if (isConnected) 
    ...
    msg.add(IMU.ax); // Acceleration X
    msg.add(IMU.ay); // Acceleration Y
    msg.add(IMU.az); // Acceleration Z
